...
download_baseline() {
    ...
    gh release download --repo "$REPO_OWNER/$REPO_NAME" --pattern 'dude.zip' \
        --dir "$WORK_DIR/baseline-download" "$latest_tag"

    mkdir -p "$WORK_DIR/baseline-dude"
    unzip -q "$WORK_DIR/baseline-download/dude.zip" -d "$WORK_DIR/baseline-dude"
    BASELINE_DIR="$WORK_DIR/baseline-dude/dude"

    if [ ! -f "$BASELINE_DIR/dude.jar" ]; then
        echo "ERROR: dude.jar not found in release archive"
        exit 1
    fi

    # The standard release doesn't ship languages.yml or .ignore — copy from repo
    cp "$PROJECT_ROOT/languages.yml" "$BASELINE_DIR/languages.yml"
    cp "$PROJECT_ROOT/.ignore" "$BASELINE_DIR/.ignore"
...
build_current() {
    ...
    mkdir -p "$WORK_DIR/current-dude/dude/results"
    cp target/dude.jar "$WORK_DIR/current-dude/dude/dude.jar"
    cp languages.yml "$WORK_DIR/current-dude/dude/languages.yml"
    cp .ignore "$WORK_DIR/current-dude/dude/.ignore"
    CURRENT_DIR="$WORK_DIR/current-dude/dude"
...
run_dude() {
    ...
    pushd "$dude_dir" > /dev/null

    DUDE_PROJECT_NAME="$repo_name" \
    DUDE_PROJECT_FOLDER="$repo_path" \
    DUDE_RESULTS_FOLDER="$results_out" \
    DUDE_LINGUIST_FILE="$dude_dir/languages.yml" \
    DUDE_IGNORE_FILE="$dude_dir/.ignore" \
    java -Xmx4g -jar dude.jar || true
...
<testsuites name="DuDe Regression Test" tests="$TOTAL_TESTS" failures="$TOTAL_FAILURES">
...
write_empty_report() {
    ...
    cat > "$REPORT_DIR/regression-results.xml" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
<testsuites name="DuDe Regression Test" tests="1" failures="0">
...
main() {
    ...
    for repo_url in "${TEST_REPOS[@]}"; do
        ...
        run_dude "$BASELINE_DIR" "$WORK_DIR/repos/$repo_name" "baseline"
        run_dude "$CURRENT_DIR"  "$WORK_DIR/repos/$repo_name" "current"
